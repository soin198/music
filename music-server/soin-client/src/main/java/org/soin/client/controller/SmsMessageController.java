...
public class SmsMessageController implements ISmsMessageApi {
     ...
     */
    @Override
    public GenericResponse<Boolean> generateCode(String phone) {
        RunTimeTool.printMethodMsg("generateCode", "生成验证码发送手机", phone);
        Assert.isBlank(phone, "请输入手机号码");
        boolean isOpen = smsMessageService.generateCode(phone);
